...
;; Version: $Id: match.lisp,v 1.4 2007/08/21 01:54:20 mwk3 Exp $
...
(define-setf-expander setf-able-progn (&rest forms &aux (store(gensym)))
   (values () () (list store)
          `(progn ,@(butlast forms) (setf ,@(last forms) ,store))
          `(progn ,@forms)))

(defun setf-able-progn (&rest args)
        (first (last args)))

(defmacro r1 (x)
  "To make (r1 x) a right-hand and left-hand function, as in E-I&G"
  `(setf-able-progn
     (unless (assoc ,x r1) (setf r1 (cons (list ,x) r1)))
     (cdr (assoc ,x r1))))

(defmacro r2 (x)
  "To make (r2 x) a right-hand and left-hand function, as in E-I&G"
  `(setf-able-progn
     (unless (assoc ,x r2) (setf r2 (cons (list ,x) r2)))
     (cdr (assoc ,x r2))))
...
(defun uvbrtrap (x)
  ...
  ;; The binding pair (y . (uvbr z)) will only occur in the source binding,
  ;; and means that y is bound to itself, and z in the target binding is bound to y,
    ...
    (do.fcs (ri nsi (down.fcs term) (first (find-or-build ans)))
	    (do.ns (ti nsi)
		  ...
		  (if (eql (r1 ti) 'done)
		      (if (eql (s1 ti) 'loop)
			  (throw 'unifytrap nil)	; loop
			  (setf ans (insert.cs (new.c ri (makeone.ns (s1 ti))) ans)))
		      (if (r1 ti)
			  (setf ans (insert.cs (new.c ri (makeone.ns (vere ti r1 r2 s1 s2))) ans))
			  (setf ans (insert.cs (new.c ri (makeone.ns ti)) ans))))
		  (if (ispat.n ti)
		      (setf ans (insert.cs (new.c ri (makeone.ns (term-vere ti r1 r2 s1 s2))) ans))
		      (setf ans (insert.cs (new.c ri (makeone.ns ti)) ans))))))))
...
(defun match (tnode &optional initbind)
  ...
  (if (find-if #'(lambda (n) (sneps:dominates.n tnode n))
		 initbind
		 :key #'cdr)
      nil
...
(defun applysubst (pnode sbst)
  ...
  (case (node-type pnode)
    ...
    (:var (if (bboundp pnode sbst) (bindingOf pnode sbst) pnode))
    (:pat (cond ((bboundp pnode sbst) (bindingOf pnode sbst))
		(t (clrhash *ssource*)
		   (clrhash *starget*)
