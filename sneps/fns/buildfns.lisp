...
;; Version: $Id: buildfns.lisp,v 1.5 2008/03/19 21:41:44 mwk3 Exp $
...
(defun canonicalCS (cs)
  ...
  (cond
   (;; Negation
    (and
     (ismembrel.cs 'max cs)
     (lisp:= (node-to-number.n (choose.ns (nodeset.cs 'max cs))) 0)
     (lisp:= (cardinality.ns (nodeset.cs 'arg cs)) 1))
   ...
   (;; and-entaliment with only one antecedent
    ...
    (and
     (ismembrel.cs '&ant cs)
     (lisp:= (cardinality.ns (nodeset.cs '&ant cs)) 1)
