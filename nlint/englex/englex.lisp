...
;; Version: $Id: englex.lisp,v 1.3 2007/08/21 01:54:21 mwk3 Exp $
  ...
  Checks is word l contains a recognized suffix"
    (cond ((string= l "") nil)
	  ...
	  ;; "NCE" is not currently implemented.
          ;((and (lisp:> (length l) 2)(is-sub-str "@@nce" l))
...
(defun mem* (item list)
  ...
  (if list
      (or (null item)
	  (not (null (lisp::member item list :test #'equal :key #'second)))
...
(defun definewords (words &aux (word (car words)) (lpairs (cdr words)) rt)
  ...
  (dolist (lpair lpairs (list word))
	...
	(unless (or (stringp feature)
	  ...
	  (lisp::push feature *FEATURE-VALUES*))
	))))
