...
;; Version: $Id: load-sneps.lisp,v 1.17 2007/10/13 21:05:28 mwk3 Exp $
...
(defvar *config-file* "F:\\GSNePS\\sneps_config.lisp"
"The location of the SNePS configuration file.")

;;;;;;;;;;;;;;;;;;;; End user custimization ;;;;;;;;;;;;;;;;;;;;;;;;

(defvar *sneps-directory* "/home/nour/Sneps-2.7.0"
...
(defvar *use-gui-show* t
  "Specifies whether the new SnePS GUI should be used for displaying
networks after the show command is called, or DOT.")

(defvar *jung-directory* "./"  
    "The directory that contains the JUNG jar files.")

(defvar *colt-directory* "./"
  "The directory containing the Colt jar files.")

(defvar *xerces-directory* "./"
  "The directory containing the Xerces jar files.")

(defvar *commons-directory* "./"
  "The directory containing the Commons jar files.")

(defvar *jimi-directory* "./"
  "The directory containing the jimi jar files.")

(defvar *eps-dump-directory* nil
  "The directory containing the eps dump jar files.")

(defvar *force-jlinker-config* nil
  "If 't', forces the jl-config file to be loaded on windows systems, which 
typically don't need to configured to use jlinker.")
...
(defvar *sneps-debug* nil
  "Set nil for compiled code that is faster, but harder to debug. Set
t for compiled code that is easier to debug, but slower.")
...
(defparameter *sneps-version*
    (format nil "SNePS-2.7 [PL:0 ~a]"
      (let ((date-string "$Date: 2007/10/13 21:05:28 $"))
