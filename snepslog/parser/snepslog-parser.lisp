...
;; Version: $Id: snepslog-parser.lisp,v 1.1 2007/10/05 17:27:59 mwk3 Exp $
...
;;;                        threshTerm | atomicTerm | gradedTerm
...
;;;                        _(_ wff _)_
;;;   gradedTerm       ::= g_(_wff, i _)_ ;; i is a positive integer
...
(defun prefixedTerm ()
  ...
  (unless (emptyInput?)
    (case (elt *Input* 0)
      (#\~ (negatedTerm))
      ;(#\g (gradedTerm))
      (#\a (or (andorTerm) (allTerm) (atomicTerm)))
      (#\n (or (nexistsTerm) (atomicTerm)))
...
(defun gradedTerm ()
  "Parse and return a graded term" 
  ;; gradedTerm ::= g _(_ wff, <lisp integer> _)_
  (when (string-equal (subseq *Input* 0 2) "g(")
    (popInput 2)
    (let ((wff (wff)))
    (when wff
      (if (firstCharp #\,) (popInput 1) (parseError "Wff should be followed by ,: ~S"))
      (loop while (digit-char-p (elt *Input* 0)) do
        (popInput 1)
      )  
     (if (firstCharp #\)) (popInput 1) (parseError "Missing closing paranthesis at: ~S"))
    `(sneps:build sneps:prop ,wff sneps:grade 2)
    )
    )
  ))
...
(defparameter *ParsingFunctions*
    '( snepslog-read-from-string %-command SNePSLOGsymbol ^-command
      ...
      negatedTerm gradedTerm nexistsParameter nexistsParameters nexistsTerm
