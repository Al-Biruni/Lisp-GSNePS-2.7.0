...
;; Version: $Id: rule-eintr.lisp,v 1.3 2007/08/21 01:54:42 mwk3 Exp $
...
(defun compute-new-support.intr (fns)
  (let* ((newsupport (new.sup))
	...
	((null s1) newsupport)
	  (setq cts (second s1))
	  (setq telprops (new.ns))
      (progn 
      	(dolist (ct cts)
      		(setq telprops (sneps:union.ns (sneps:context-telprops ct) telprops))
      	)
      	(setq newsupport
	    (insert.sup (if (equal *TYPE* 'AND)
			    (combine-and-intr-ots* (first s1) (second s1))
			    (combine-ots* (first s1)))
			(fullbuildcontext (new.ns) (second s1) (new.ns) telprops)
			newsupport))))))
