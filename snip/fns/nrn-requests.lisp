...
;; Version: $Id: nrn-requests.lisp,v 1.9 2007/09/04 15:50:05 mwk3 Exp $
...
;													nea  8/15
;
(defparameter *depthCutoffBack* 5
  "Maximum height of SNIP subgoal.")

(defun send-request (req n restr)
 ;(format t "~% dest = ~s node=~s  request = ~s ~% " n *NODE* req)	
...
; added handling requests of graded propositions nea
;
;
    ...
    (broadcast-request
	  ...
	  (or pathfrom-result
	      (setq pathfrom-result
		    (sneps::pathfrom '(sneps::or cq- arg- dcq- if- prop-)
				     *NODE*)))
...
(defun send-node-to-node-requests (restriction context)
  ...
  (let* ((rsbst (subst.restr restriction))
    ...
    (if (and (isassert.n rnode context) (not(eq rnode (sneps:negate.n *NODE*)) ) )
