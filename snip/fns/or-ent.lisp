...
;; Version: $Id: or-ent.lisp,v 1.3 2007/08/21 01:54:41 mwk3 Exp $
...
(defun rule-handler.v-ent (ant-report cqch)
  ...
  ;(format t "~% ruis = ~s" (is-all-pat-same-vars (ants.cqch cqch)))
  (let ((ruis (if (is-all-pat-same-vars (ants.cqch cqch))
	...
	(let ((restr (make.restr (subst.rui rui)))
	  ...
	  ;(format t "~% rule node = ~s ant= ~s" *NODE* (signature.rep ant-report))
