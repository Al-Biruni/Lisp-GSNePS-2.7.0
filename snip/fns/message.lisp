...
;; Version: $Id: message.lisp,v 1.3 2007/08/21 01:54:40 mwk3 Exp $
...
;                       unknown one, a new node is built.
  ...
  (let* ( (d (destination.ch ch))
	 ...
	 (situation
	  (cond ((is-user.dest d) 'to-user)
		...
		((let ((*node* d)) (is-grading-to-prop.rep rep)) 'to-graded)
		(t 'to-nonrule)))
	 (newsub (case situation
		   ((to-nonrule)
		    (match::substituteinterms (switch.ch ch) (subst.rep rep)))
		   ((to-rule to-user from-cq) (subst.rep rep))
		   ((to-cq to-graded)  (match::substituteinterms (match::initbind d) (subst.rep rep)))))
	 )
			 ...
			 ((to-user to-cq)
			  (if (and (eq *name* 'rule)
				   ...
				   (instanciated-quantified-vars.n
				    signature ch newsub))
			      (change-tag-support (support.rep rep) 'sneps:HYP 'sneps:DER)
			    (support.rep rep))
			  )
			 ((to-graded) (support.rep rep))
			 ((to-nonrule)
	...
	(when (and (or (ismol.n newnode) (ispat.n newnode))
		   (not (and (equal *name* 'rule)
			     ...
			     (eq newnode *node*)))
		    (not (eq newnode (sneps:negate.n *NODE*)))
		    (not (and(ispat.n *NODE*) (isneg.rep rep)))
		   )

	 
	  (progn  (when (eq situation 'snip::to-graded)
	  	(setq support (generate-tel-support.sup newnode support signature (context.rep rep)))
	  	(remark '"~%I know" (makeone.ns signature) nil)
	  	(remark '"~%Hence, I infer" (makeone.ns newnode) nil)
	  	) 
	...
	;(jcall (jmethod "TestAPI" "testJava") (jclass "TestAPI"))
	(make.rep newsub
		  support
		  (sign.rep rep)
		  signature
