...
;; Version: $Id: report.lisp,v 1.3 2007/08/21 01:54:36 mwk3 Exp $
...
;       returns       : <boolean>
;
;       description   : determines whether or not the report is from a grading
;                       node to a graded node
;
;       implementation: checks to see if the report was sent by a node which
;                       is at the end of a prop- arc
;
;                                        written :  nea  7/15

(defmacro is-grading-to-prop.rep (report)
  (print report)
  `(let ((sender (signature.rep ,report)))
      (and (is.n sender)
           or (ismemb.ns sender (nodeset.n *NODE* 'prop-)))))
;
;
; =============================================================================
;
...
(defmacro is-node-to-node.rep (report)
  `(and (is.n (signature.rep ,report))
        ...
        (not (is-grading-to-prop.rep ,report))
        ))
;
;
;
