...
/**
 * Represents an instance of a connection between Java and the SNePS
...
public class JavaSnepsAPI
{
	...
	(new Thread(jlc)).start();
	try{
	    Runtime.getRuntime().exec(sneps_command + " -- -config-file " + 
				      sneps_config +
    ...
    **/
    ...
    /** Given a valid ask input, invokes the SNePSLOG ask method on that input. Returns an array of strings representing the node results.
     * @param command Valid ask input with which to query SNePS.
     * @return An array of String objects representing query results 
	...
	try {
	    snepslog_ask.call();
	    result = snepslog_ask.stringArrayValue();
	}
	catch (JavaLinkDist.JLinkerException ex) {
	    ex.printStackTrace();
	} // end of catch
    ...
    /** Given a valid ask input, invokes the SNePSLOG askifnot method on that input. Returns an array of strings representing the node results.
     * @param command Valid ask input with which to query SNePS. 
     * @return An array of String objects representing query results 
     * @see "The SNePS 2.7.0 Manual - snepslog:askifnot"
     **/
    public String[] askifnot (String command)
    {
	String [] result = null;
	TranStruct ts = null;
    ...
    /** Given a valid ask input, invokes the SNePSLOG askwh method on that input. Returns an array list of the substitution objects representing the variable substitutions needed to answer the query.
     * @param command Valid ask input with which to query SNePS. 
     * @return {@link java.util.ArrayList} of {@link edu.buffalo.sneps.Substitution}
     ...
     **/
    public ArrayList askwh (String command)
    {
	...
	try {
	    snepslog_ask.call();
	    result = (ArrayList)snepslog_ask.objectValue();
	}
	catch (JavaLinkDist.JLinkerException ex) {
	    ex.printStackTrace();
	} // end of catch
    ...
    /** Given a valid ask input, invokes the SNePSLOG askwhnot method on that input. Returns an array list of substitution objects representing the variable substitutions needed to answer the query.
     * @param command Valid ask input with which to query SNePS. 
     * @return  {@link java.util.ArrayList} of {@link edu.buffalo.sneps.Substitution}
     * @see edu.buffalo.sneps.Substitution#getValFromVar(String) "The get value from variable method"
     * @see "The SNePS 2.7.0 Manual - snepslog:askwhnot"
     **/
    public ArrayList askwhnot (String command)
    {
	//System.out.println("Asking the system (askwh): "+ command);
	ArrayList result = null;
