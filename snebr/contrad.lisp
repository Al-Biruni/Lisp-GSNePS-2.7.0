...
;; Version: $Id: contrad.lisp,v 1.4 2007/08/21 01:54:23 mwk3 Exp $
...
;                                  modified:  nea     7/15    ;; handling graded propositions
;
...
(defun ck-contradiction-1 (newnode contr-nd context flag)
  (let ((contrsup (sneps:ctcs-to-cts (sneps:node-asupport contr-nd)))
        (nodetelsup (sneps:getcontextset.ctcs 'sneps:TEL (sneps:node-asupport newnode)))
        (contrtelsup (sneps:getcontextset.ctcs 'sneps:TEL (sneps:node-asupport contr-nd)))
    )
    (compute-node-grade newnode)
    (compute-node-grade contr-nd)

    ;(print (sneps:node-grade newnode))
    ;(print (sneps:node-grade contr-nd))

    (snip:remark '"~%Contradiction derived between " (makeone.ns newnode) nil)
    (snip:remark '"and " (makeone.ns contr-nd) nil)

    (setq kernels (list))
    (when (not (or (null nodetelsup) (null contrtelsup)))  ;; both are graded
           (snip:remark '"~%Both contradicting nodes are graded. The grade of " (sneps:makeone.ns newnode) nil)
           (snip:remark (format nil "is ~s. The grade of" (sneps:node-grade newnode)) (sneps:makeone.ns contr-nd) nil)
           (snip:remark (format nil "is ~s. ~%" (sneps:node-grade contr-nd)) nil nil)
           (setq kernels (append kernels (list(sneps:make.ns newnode contr-nd))))
    )
    
    
    (when (not (null nodetelsup))
      (setq nodeset new.ns)
      (setq cts (sneps:getcontextset.ctcs 'sneps:DER (sneps:node-asupport contr-nd)))
      (dolist (ct cts)
        (setq nodeset (sneps:insert.ns newnode (sneps:new.ns)))
        (setq nodeset (sneps:union.ns nodeset (sneps:context-telprops ct)))
        (setq kernels (append kernels (list nodeset)))
      )
    ) 
    
    (when (not (null contrtelsup))
      (setq nodeset new.ns)
      (setq cts (sneps:getcontextset.ctcs 'sneps:DER (sneps:node-asupport newnode)))
      (dolist (ct cts)
        (setq nodeset (sneps:insert.ns contr-nd (sneps:new.ns)))
        (setq nodeset (sneps:union.ns nodeset (sneps:context-telprops ct)))
        (setq kernels (append kernels (list nodeset)))
      )
    )
   ...
   ;; both are not graded
    (setq cts1 (sneps:getcontextset.ctcs 'sneps:DER (sneps:node-asupport newnode)))
    (setq cts2 (sneps:getcontextset.ctcs 'sneps:DER (sneps:node-asupport contr-nd)))
    (dolist (ct1 cts1)
            (dolist (ct2 cts2)
                (setq kernels (append kernels (list(sneps:union.ns (sneps:context-telprops ct1)(sneps:context-telprops ct2)))))
            )
    )
    (snip:remark (format nil "~%The construced bottom kernels are ~s.~%" kernels) nil nil)
    (snip:remark (format nil "~%Removing the proposition with the least grade from each of the kernels from the telprops of the crntct.~%") nil nil)
    (dolist (k kernels)
        (setq mins (get-min-grade k))
         (snip:remark (format nil "~%From ~s, ~s will be removed.~%" k mins) nil nil)
        (dolist (n mins)
          (setf (sneps:context-telprops context) (sneps:compl.ns (sneps:context-telprops context) (sneps:makeone.ns n)))
        )

    )
        
    (if (and (isassert.n newnode context) (isassert.n contr-nd context))
     ;both nodes are not graded and have no telescoped supports - let the user resolve the contr
    (progn 
    (update-contexts (ctcs-to-cts (sneps:node-asupport newnode)) contrsup) 
    (sneps:mark-inconsistent context)
    (sneps:updateall (sneps::get-context context))
...
(defun get-min-grade (ns)
  (setq mins (sneps:new.ns))
  (setq min  1000)
  (dolist (n ns)
    (compute-node-grade n)
    (when (<= (sneps:node-grade n) min) (setq min (sneps:node-grade n)))
  )
  (dolist (n ns)
    (when (eq (sneps:node-grade n) min)
      (setq mins (sneps:insert.ns n mins))
    )
  )
  mins)
;
...
;
;       arguments     : n -<node>
;                     : mode - <integer> (optional) representing the plus operator
;                       default mode is maximum operator
;
;       returns       : grade
;
;       description   : computes the grade of node n in a certain grading chain
;                       
;
;                       
;                                        written :  nea 7/15
...
(defun compute-node-grade (n &optional mode)
(let ((cts (sneps:getcontextset.ctcs 'sneps:TEL (sneps:node-asupport n))))
(setq max 0)
(dolist (ct cts)
  (setq ctgrade (gethash (context-order ct) (node-contexts-grades n)))
  (when (and (not (eq ctgrade nil)) (> ctgrade max))
    (setq max ctgrade)
  ) 
)
(setf (sneps:node-grade n) max)
)
)
;
;
